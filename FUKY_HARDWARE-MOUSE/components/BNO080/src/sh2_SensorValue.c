...
static int decodeLinearAcceleration(sh2_SensorValue_t *value, const sh2_SensorEvent_t *event)
{
    value->un.linearAcceleration.x = read16(&event->report[4]) ;//* SCALE_Q(8)去掉了，因为缩放放到了电脑端处理
    value->un.linearAcceleration.y = read16(&event->report[6]) ;
    value->un.linearAcceleration.z = read16(&event->report[8]) ;
    return SH2_OK;
}
...
static int decodeRotationVector(sh2_SensorValue_t *value, const sh2_SensorEvent_t *event)
{
    value->un.rotationVector.i = read16(&event->report[4]) ;//* SCALE_Q(14);在电脑端处理
